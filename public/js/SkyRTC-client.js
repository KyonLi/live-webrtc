var SkyRTC = function () {
    ...
    var nativeRTCIceCandidate = (window.RTCIceCandidate || window.webkitRTCIceCandidate || window.mozRTCIceCandidate);
    var nativeRTCSessionDescription = (window.RTCSessionDescription || window.webkitRTCSessionDescription || window.mozRTCSessionDescription); // order is very important: "RTCSessionDescription" defined in Nighly but useless
    var packetSize = 1000;
    ...
    function skyrtc() {
        ...
        //保存所有与本地相连的windowId， 键为socket id，值为windowId
        this.windowIds = {};
        //保存所有与本地连接的socket的id
        this.connections = [];
        //初始时需要构建链接的数目
    ...
    //本地连接信道，信道为websocket
    skyrtc.prototype.connect = function (server, room, windowId) {
        ...
        this.on('_peers', function (data) {
            //获取所有服务器上的
            console.log(data);
            that.windowIds = data.windowIds;
            that.me = data.you;
            //如果是主播端获取所有连接，如果是刮票口只获取主播端
            if (that.windowId == 0) {
                that.connections = data.connections;
            } else {
                that.connections.length = 0;
                for (let index = 0; index < data.connections.length; index++) {
                    if (data.windowIds[data.connections[index]] == 0) {
                        that.connections.push(data.connections[index]);
                    }
                }
            }
        ...
        this.on('_new_peer', function (data) {
            ...
            that.windowIds[data.socketId] = data.windowId;
            var pc = that.createPeerConnection(data.socketId);
            //自身为主播端时，不向其他实例添加本地流
            if (that.windowId == 0) {
                console.log("_new_peer 我是主播，不向其他实例添加本地流");
            } else {
                if (data.windowId == 0) {
                    console.log("_new_peer 我是刮票，只向主播添加本地流");
        ...
        this.on('_remove_peer', function (data) {
            ...
            delete that.windowIds[data.socketId];
            ...
            that.emit("remove_peer", data.socketId, data.windowId);
        });
    ...
    skyrtc.prototype.addStreams = function () {
        console.log("---------------------------------");
        console.log(this.windowIds);
        console.log(this.peerConnections);
        var connection;
        for (connection in this.peerConnections) {
            //自身为主播端时，不向其他实例添加本地流
            // if (this.windowId == 0) {
            //     console.log("我是主播，不向其他实例添加本地流");
            // } else {
            //     if (this.windowIds[connection] == 0) {
            //         console.log("我是刮票，只向主播添加本地流");
    ...
    skyrtc.prototype.createPeerConnections = function () {
        ...
        for (i = 0, m = this.connections.length; i < m; i++) {
            ...
            */
            if (this.windowId == 0) {
                console.log("我是主播，与所有用户创建连接");
                this.createPeerConnection(this.connections[i]);
            }
            else if (this.windowIds[this.connections[i]] == 0) {
                console.log("我是刮票，只与主播创建连接");
                this.createPeerConnection(this.connections[i]);
            }
        }
        console.log("---------------------------------");
    ...
    skyrtc.prototype.createPeerConnection = function (socketId) {
        ...
        pc.onaddstream = function (evt) {
            //只有主播端显示其他窗口的画面
            if (that.windowId == 0) {
                that.emit('pc_add_stream', evt.stream, socketId, that.windowIds[socketId], pc);
            }
        };
    ...
    skyrtc.prototype.addDataChannel = function (socketId, channel) {
        ...
        channel.onmessage = function (message) {
            ...
            if (json.type === '__file') {
                ...
                that.emit('data_channel_message', channel, socketId, that.windowIds[socketId], json.data);
            }
        };
